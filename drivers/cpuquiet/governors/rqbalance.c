...
static bool clusterswitch_mode; /* TODO: Not implemented */
...
static void hotplug_hmp_final_decision(unsigned int cpu, bool up)
{
	...
	unsigned int little_cores, big_cores;
	unsigned int little_max = 4; /* TODO: Get max (SoC) little cores properly */

	if (up) {
		/* Analyze online cores situation */
		big_cores = num_online_cluster_cpus(CLUSTER_BIG);
		little_cores = num_online_cluster_cpus(CLUSTER_LITTLE);

pr_info("----_RQBALANCE: BIG_CORES = %u LITTLE_CORES= %u\n", big_cores, little_cores);

#if 1
		/* If we want 4 little cores, we can instead use one
		   big core and two little */
		if (cluster == CLUSTER_LITTLE &&
		    little_cores >= little_max - 1 &&
		    big_cores == 0) {
			cpuquiet_wake_cpu(
...
#else
		if (cluster == CLUSTER_LITTLE) {
			if (little_cores >= little_max - 1 &&
			    big_cores == 0) {
				cpuquiet_wake_cpu(
					get_offline_core(CLUSTER_BIG),
							 false);
				cpuquiet_quiesence_cpu(
					get_offline_core(CLUSTER_LITTLE),
							 false);
				return;
			}
...
//CPQ_BASIC_ATTRIBUTE(idle_bottom_freq[0], 0644, uint);
//CPQ_BASIC_ATTRIBUTE(idle_top_freq[0], 0644, uint);
